...
BOOL IsWoW64()
{
	...
	if (API::IsAvailable(API_IDENTIFIER::API_IsWow64Process)) {
		auto fnIsWow64Process = static_cast<pIsWow64Process>(
			API::GetAPI(API_IDENTIFIER::API_IsWow64Process));
...
PVOID64 GetPeb64()
{
	...
	if (API::IsAvailable(
			API_IDENTIFIER::API_NtWow64QueryInformationProcess64)) {
		PROCESS_BASIC_INFORMATION_WOW64 pbi64 = {};

		auto NtWow64QueryInformationProcess64 =
			static_cast<pNtWow64QueryInformationProcess64>(API::GetAPI(
				API_IDENTIFIER::API_NtWow64QueryInformationProcess64));
		NTSTATUS status = NtWow64QueryInformationProcess64(
			GetCurrentProcess(), ProcessBasicInformation, &pbi64, sizeof(pbi64),
			nullptr);
...
{
	if (_tcschr(lpSubKey, _T('*')) == NULL &&
		_tcschr(lpSubKey, _T('?')) == NULL) {
		HKEY hkResult = NULL;
		if (RegOpenKeyEx(hKey, lpSubKey, 0, KEY_READ, &hkResult) ==
			ERROR_SUCCESS) {
			RegCloseKey(hkResult);
			return TRUE;
		}
		...
		while (1) {
			cchName = MAX_PATH;
			lResult = RegEnumKeyEx(hKeyParent, dwIndex, subkeyName, &cchName,
								   NULL, NULL, NULL, NULL);
...
BOOL check_mac_addr(const TCHAR *szMac)
{
	...
	if (dwResult == ERROR_SUCCESS) {
		// Convert the given mac address to an array of multibyte chars so we
		// can compare.
...
BOOL GetOSDisplayString(LPTSTR pszOS)
{
	...
	auto RtlGetVersion = static_cast<pRtlGetVersion>(
		API::GetAPI(API_IDENTIFIER::API_RtlGetVersion));

	bOsVersionInfoEx = RtlGetVersion((RTL_OSVERSIONINFOEXW *)&osvi);

	if (API::IsAvailable(API_GetNativeSystemInfo)) {
		auto GetNativeSystemInfo = static_cast<pGetNativeSystemInfo>(
			API::GetAPI(API_IDENTIFIER::API_GetNativeSystemInfo));
		...
		if (osvi.dwMajorVersion == 10) {
			...
			if (osvi.dwMinorVersion == 0) {
				if (osvi.wProductType == VER_NT_WORKSTATION)
					...
					StringCchCat(pszOS, MAX_PATH, TEXT("Windows Server 2008 "));
			}
			...
			auto GetProductInfo = static_cast<pGetProductInfo>(
				API::GetAPI(API_IDENTIFIER::API_GetProductInfo));
			...
			if (GetSystemMetrics(SM_SERVERR2))
				...
				StringCchCat(pszOS, MAX_PATH, TEXT("Windows Server 2003, "));
			...
			if (osvi.wProductType != VER_NT_WORKSTATION) {
				if (si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_IA64) {
					if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
						StringCchCat(pszOS, MAX_PATH,
									 TEXT("Datacenter Edition for "
										  "Itanium-based Systems"));
					else if (osvi.wSuiteMask & VER_SUITE_ENTERPRISE)
						StringCchCat(pszOS, MAX_PATH,
									 TEXT("Enterprise Edition for "
										  "Itanium-based Systems"));
				}
			...
			if (osvi.wProductType == VER_NT_WORKSTATION) {
				...
				if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
					...
					StringCchCat(pszOS, MAX_PATH, TEXT("Server"));
			}
		}
...
DWORD GetProccessIDByName(TCHAR *szProcessNameTarget)
{
	...
	for (int i = 0; i < cProcesses; i++) {
		...
		HANDLE hProcess = OpenProcess(
			PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, FALSE, processIds[i]);
			...
			do {
				if (te.dwSize >=
					...
					if (te.th32OwnerProcessID == pid) {
						HANDLE hThread =
							OpenThread(READ_CONTROL, FALSE, te.th32ThreadID);
						if (!hThread)
...
BOOL ExecWMIQuery(IWbemServices **pSvc, IWbemLocator **pLoc,
				  IEnumWbemClassObject **pEnumerator, const TCHAR *szQuery)
{
	// Execute WMI query
...
#else // x64 implementation
	{
		// Windows x64 deliberately blocks user-mode LDT access
		// SLDT instruction require kernel privileges
		return 0xdead0000;
	}
#endif
}
...
UINT enum_system_firmware_tables(DWORD FirmwareTableProviderSignature,
								 PVOID pFirmwareTableBuffer, DWORD BufferSize)
{
	...
	auto EnumSystemFirmwareTables = static_cast<pEnumSystemFirmwareTables>(
		API::GetAPI(API_IDENTIFIER::API_EnumSystemFirmwareTables));
...
{
	auto NtWow64ReadVirtualMemory64 = static_cast<pNtWow64ReadVirtualMemory64>(
		API::GetAPI(API_IDENTIFIER::API_NtWow64ReadVirtualMemory64));
	...
	if (hProcess != NULL) {
		NTSTATUS status = NtWow64ReadVirtualMemory64(hProcess, address, buffer,
													 size, &bytesRead);
		/*if (status != 0)
		printf("NTSTATUS: %x\n", status);*/
...
{
	return attempt_to_read_memory_wow64(buffer, size,
										reinterpret_cast<PVOID64>(address));
}
...
std::vector<PMEMORY_BASIC_INFORMATION64> *enumerate_memory_wow64()
{
	...
	auto NtWow64QueryVirtualMemory64 =
		static_cast<pNtWow64QueryVirtualMemory64>(
			API::GetAPI(API_IDENTIFIER::API_NtWow64QueryVirtualMemory64));
	...
	while (addr < MaxAddress) {
		...
		NTSTATUS status;
		if ((status = NtWow64QueryVirtualMemory64(
				 GetCurrentProcess(), (PVOID64)addr, 0, mbi,
				 sizeof(MEMORY_BASIC_INFORMATION64), &returnLength)) != 0) {
